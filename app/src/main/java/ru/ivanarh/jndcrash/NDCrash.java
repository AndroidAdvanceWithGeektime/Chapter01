...
public class NDCrash {
   ...
   * @return Error status.
   */
  public static NDCrashError initializeInProcess(
          @Nullable String crashReportPath, NDCrashUnwinder unwinder) {
    return NDCrashError.values()[nativeInitializeInProcess(crashReportPath, unwinder.ordinal())];
  }

  /// Native implementation method.
  private static native int nativeInitializeInProcess(
      @Nullable String crashReportPath, int unwinder);
   ...
   * @param crashReportPath Path where a crash report is saved.
   * @param unwinder Used unwinder. See ndcrash_unwinder type in ndcrash.h.
   * @param serviceClass Class of background service. Used when we need to use a custom subclass of
   *     NDCrashUnwinder to use as a background service. If you didn't subclass NDCrashUnwinder,
   *     please pass NDCrashUnwinder.class.
   * @return Error status.
   */
  ...
  static NDCrashError startOutOfProcessDaemon(
    ...
    final NDCrashError result =
        NDCrashError.values()[
            nativeStartOutOfProcessDaemon(
                getSocketName(context), crashReportPath, unwinder.ordinal())];
  ...
  private static native int nativeStartOutOfProcessDaemon(
      @NonNull String socketName, @Nullable String crashReportPath, int unwinder);
  ...
  @Nullable private static volatile OnCrashCallback mOnCrashCallback = null;

  /** Background service class for out-of-process mode. */
  @Nullable private static Class<? extends NDCrashService> mServiceClass = null;
  ...
  /**
   * Retrieves a socket name from Context instance. We use a package name with additional suffix to
   * make sure that socket name doesn't intersect with another application.
   *
  ...
  public interface OnCrashCallback {
    ...
    /**
     * Runs when crash is detected. This method is run from background (daemon) thread. This method
     * allows to process a report immediately after crash.
     *
     * @param reportPath Path to file containing crash report.
     */
