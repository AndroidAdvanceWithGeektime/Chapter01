...
JNIEXPORT jint JNICALL Java_ru_ivanarh_jndcrash_NDCrash_nativeInitializeInProcess(
    ...
    const enum ndcrash_error error = ndcrash_in_init((enum ndcrash_unwinder) unwinder,
                                                     crashReportPath);
    if (jCrashReportPath) {
...
JNIEXPORT jint JNICALL Java_ru_ivanarh_jndcrash_NDCrash_nativeInitializeOutOfProcess(
    ...
    const char *socket_name = jSocketName ? (*env)->GetStringUTFChars(env, jSocketName, NULL)
                                          : NULL;
...
static void jndcrash_on_crash(const char *report_file, void *argvoid) {
    jndcrash_callback_arg_t *const arg = (jndcrash_callback_arg_t *) argvoid;
    const jstring j_report_file = (*arg->daemon_thread_env)->NewStringUTF(arg->daemon_thread_env,
                                                                          report_file);
...
JNIEXPORT jint JNICALL Java_ru_ivanarh_jndcrash_NDCrash_nativeStartOutOfProcessDaemon(
    ...
    arg->jm_runOnCrashCallback = (*env)->GetStaticMethodID(env, arg->jc_NDCrash,
                                                           "runOnCrashCallback",
                                                           "(Ljava/lang/String;)V");
...
Java_ru_ivanarh_jndcrash_NDCrash_nativeStopOutOfProcessDaemon(JNIEnv *env, jclass type) {
#ifdef ENABLE_OUTOFPROCESS
